...
function Service() {
  ...
  const [isSubmitting, setIsSubmitting] = useState(false); // حالة للتحكم في الزر أثناء الطلب

  const handleBooking = async () => {
    setIsSubmitting(true); // تعيين حالة الزر إلى معالجته
    try {
      const res = await newRequest.post(`/bookings/${id}`);
      console.log("Order created successfully", res.data);
      setIsSubmitting(false); // تعيين حالة الزر إلى مكتمل
    } catch (err) {
      console.log(err);
      setIsSubmitting(false); // تعيين حالة الزر إلى مكتمل حتى في حالة الفشل
    }
  };
            ...
            <div className=" md:block">
              <div className="pl-10">
                    <Button
                      ...
                      disabled={isSubmitting} // تعطيل الزر أثناء معالجة الطلب
                    >
                      <NotebookPen/>
